...
// Contruct a linear regression model
const model = tf.sequential();
model.add(tf.layers.dense({inputShape: [1], units: 1}));
// Model complilation
const optimizer = tf.train.sgd(0.003); // slower learning rate
model.compile({optimizer: optimizer, loss: 'meanAbsoluteError'});

/**
 * Be sure to wait for the promise returned from model.fit to resolve before executing model.evaluate
 */
...
async function training() {
    await model.fit(trainTensors.sizeMB,
                    ...
                    {epochs: 200});
    await model.evaluate(testTensors.sizeMB, testTensors.timeSec).print();
    await model.predict(testTensors.sizeMB).print();
}

// Calculate the loss function
// model.evaluate(testTensors.sizeMB, testTensors.timeSec).print();
// model.predict(testTensors.sizeMB).print();
