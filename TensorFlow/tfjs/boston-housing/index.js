...
export function arraysToTensors() {
    ...
    tensors.testFeatures = normalization.normalizeTensor(tensor.rawTestFeatures, dataMean, dataStd);
}
...
/**
 * Builds and returns Multi Layer Perceptron Regression Modle
...
export function describeKernelElements(kernel) {
    tf.util.assert(
        kernel.length == 12,
        `kernel must be an array of length 12, got ${kernel.length}`
    );
    const outList = [];
